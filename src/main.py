...
if __name__ == '__main__':    
    ...
    fileName, fileExtension = args.input.split('.')

    if fileExtension == 'slpd':
        subprocess.run(['slpdgen', '-json', args.input])

    with open(fileName + '.json') as f:
        proto = SlpdProto(json.load(f))

    if args.command == 'dissector':
        dissector.generate(proto, fileName, args.port)

    if args.command == 'py-proto':
        python.generate(proto, fileName)
